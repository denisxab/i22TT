...
- Протестировать команды `parse`/`merge` в консоли
- Создать алгоритм получения слов на необходимый язык из общего файла
- Осуществлять перевод на необходимый язык
- Создать API для работы всей системы
